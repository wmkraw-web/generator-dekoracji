...
# --- GENERATOR PDF (FIXED POSITIONING) ---
def create_pdf_v82(mode, items, col, za_co, data, tytul, styl):
    ...
    for name in items:
        ...
        if mode == 'dyp':
            ...
            # WYŚRODKOWANIE LITERY (ZAMIAST SZTYWNEGO XY)
            pdf.set_y(50)
            if styl == "Kontur":
                pdf.set_text_color(255, 255, 255) # Biały środek
                # Prawidłowy sposób na kontur w nowym fpdf2
                pdf.set_draw_color(r, g, b)
                pdf.set_line_width(1.5)
                try:
                    # To jest magia fpdf2 - render mode 1 to outline (kontur)
                    pdf.set_text_render_mode(render_mode=1)
                    pdf.cell(190, 210, txt, align='C')
                    pdf.set_text_render_mode(render_mode=0) # powrót do fill
                except:
                    # Jeśli serwer ma starą bibliotekę - fallback na kolor pełny
...
if nav == "🔠 Napisy":
    ...
    with c1:
        ...
        if st.button("GENERUJ PDF"):
            if name_list:
                out_l = create_pdf_v82('lit', name_list, kol_l, "", "", "", styl_l)
    ...
    with colB:
        ...
        if st.button("GENERUJ DYPLOMY"):
            out_d = create_pdf_v82('dyp', name_list_d, kol_d, final_tresc, miejsc, okazja, "")
