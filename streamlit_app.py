...
def draw_background_pattern(pdf, r, g, b, orientation):
    pdf.set_draw_color(r, g, b)
    pdf.set_fill_color(r, g, b)
    max_x = 297 if orientation == 'L' else 210
    max_y = 210 if orientation == 'L' else 297
    for _ in range(30):
        pdf.set_alpha(0.1)
...
def generate_master_pdf(mode, items, col, za_co, data, tytul, styl):
    ...
    for name in items:
        ...
        if mode == 'dyp':
            ...
            pdf.set_text_color(r, g, b); pdf.set_font(fn, size=50 if fn=="Roboto" else 45)
            ...
            if styl == "Kontur":
                pdf.set_text_color(255, 255, 255); pdf.set_draw_color(r, g, b); pdf.set_line_width(1.5)
                pdf._out("1 Tr")
...
if narzedzie == "Napisy ścienne":
    ...
    with c1:
        st.session_state.napis_txt = st.text_input("Hasło napisu:", value=st.session_state.napis_txt)
        ...
        if st.button("GENERUJ NAPISY"):
            ...
            st.download_button("📥 Pobierz plik PDF", out, "napisy.pdf")
    with c2:
    ...
    okazja = st.selectbox("Wybierz okazję:", list(RYMOWANKI.keys()))
    if st.button("✨ GENERUJ RYMOWANKĘ"):
    ...
    with c1:
        ...
        tresc_d = st.text_area("Treść dyplomu:", value=st.session_state.get('ai_text', 'za wzorowe zachowanie'))
    with c2:
